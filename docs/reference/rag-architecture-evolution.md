# RAG 아키텍처 발전사: Naive RAG에서 Ontology-Enhanced & Agentic RAG까지

> Retrieval-Augmented Generation(RAG)은 LLM의 파라메트릭 지식 한계를 외부 검색으로 보완하는 패러다임이다.
> 본 문서는 2020년 RAG 최초 제안부터 2025년 Ontology-Enhanced RAG 및 Agentic RAG에 이르기까지의
> 아키텍처 발전 흐름을 공식 논문 기반으로 분석한다.
...
timeline
    ...
    section Ontology-Enhanced RAG
        2024~2025 : OG-RAG (Sharma et al., EMNLP 2025)
                  : 온톨로지 기반 하이퍼그래프 검색
        2025 : KG-RAG (Wang et al., Scientific Reports 2025)
             : KG + Dense 이중 채널 검색
        2025 : KG2RAG (Jiang et al., NAACL 2025)
             : KG 구조 기반 쿼리 재구성
...
flowchart LR
    ...
    L7["한계: 도메인 구조 지식<br/>미반영, 사실 검증 부족"]
    ...
    style OGRAG fill:#16A085,color:#fff
    ...
    style L7 fill:#fff,stroke:#E74C3C,color:#333
```
...
flowchart LR
    ...
    --> O["Ontology-Enhanced RAG<br/>도메인 온톨로지 기반<br/>구조적 사실 검증"]
...
### 2.12 Ontology-Enhanced RAG (2024~2025)

**기원 논문**

- **OG-RAG**: Sharma, K., Kumar, P., & Li, Y., *"OG-RAG: Ontology-Grounded Retrieval-Augmented Generation for Large Language Models"*, EMNLP 2025 ([ACL Anthology](https://aclanthology.org/2025.emnlp-main.1674/))
- **KG-RAG**: Wang, S., Yang, H. & Liu, W., *"Research on the construction and application of retrieval enhanced generation (RAG) model based on knowledge graph"*, Scientific Reports, 15, 40425, 2025 ([Nature](https://www.nature.com/articles/s41598-025-21222-z))
- **KG2RAG**: Jiang et al., *"Knowledge Graph-Guided Retrieval Augmented Generation"*, NAACL 2025 ([arXiv:2502.06864](https://arxiv.org/abs/2502.06864))
- **Survey**: Peng et al., *"Graph Retrieval-Augmented Generation: A Survey"*, ACM TOIS, 2025 ([ACM](https://dl.acm.org/doi/10.1145/3777378))
- **비교 연구**: *"Ontology Learning and Knowledge Graph Construction: A Comparison of Approaches and Their Impact on RAG Performance"*, 2025 ([arXiv:2511.05991](https://arxiv.org/abs/2511.05991))

> 아래 다이어그램은 Ontology-Enhanced RAG의 3가지 대표적 접근법을 비교한 것이다.
> OG-RAG는 하이퍼그래프 기반, KG-RAG는 이중 채널 검색 기반, KG2RAG는 서브그래프 추출 기반으로
> 각각 도메인 온톨로지를 RAG 파이프라인에 통합한다.

**OG-RAG 아키텍처 (Sharma et al., EMNLP 2025):**

```mermaid
flowchart TB
    subgraph Indexing["Ontology-Grounded Indexing (Offline)"]
        DOC["Domain Documents"] --> CHUNK["Text Chunking"]
        ONT["Domain Ontology<br/>(entities, relations, constraints)"] --> GROUND["Ontology Grounding"]
        CHUNK --> GROUND
        GROUND --> HG[("Hypergraph<br/>각 Hyperedge = 온톨로지 기반<br/>사실 지식 클러스터")]
    end

    subgraph Retrieval["Ontology-Guided Retrieval"]
        Q["Query"] --> OPT["Optimization Algorithm<br/>(최소 Hyperedge 집합 선택)"]
        HG --> OPT
        OPT --> CTX["Minimal Grounded Context"]
    end

    subgraph Generation["Grounded Generation"]
        CTX --> LLM["LLM Generator"]
        Q --> LLM
        LLM --> ANS["Factually Grounded Answer<br/>+ Attribution"]
    end

    style Q fill:#3498DB,color:#fff
    style ONT fill:#16A085,color:#fff
    style HG fill:#9B59B6,color:#fff
    style ANS fill:#2ECC71,color:#fff
...
**KG-RAG 이중 채널 아키텍처 (Wang et al., Scientific Reports 2025):**

```mermaid
flowchart TB
    Q["Query"] --> CH1["Channel 1: Dense Passage Retrieval<br/>(DPR 기반 벡터 검색)"]
    Q --> CH2["Channel 2: Graph Neural Network<br/>(KG 내 의미 경로 구조적 검색)"]

    subgraph KnowledgeGraph["Knowledge Graph"]
        direction LR
        E1["Entity A"] --> |"relation"| E2["Entity B"]
        E2 --> |"relation"| E3["Entity C"]
        E1 --> |"relation"| E3
    end

    KnowledgeGraph --> CH2
    CH2 --> PA["Path Attention Mechanism<br/>(관련 엔티티-관계 체인 필터링)"]
    CH1 --> MERGE["Knowledge Injection Module"]
    PA --> MERGE
    MERGE --> GEN["Generator (LLM)"]
    Q --> GEN
    GEN --> ANS["Answer"]

    style Q fill:#3498DB,color:#fff
    style KnowledgeGraph fill:#16A085,color:#fff
...
```mermaid
flowchart LR
    subgraph OGRAG["OG-RAG"]
        direction TB
        OG1["온톨로지로 문서를<br/>하이퍼그래프 구조화"]
        OG2["최적화 알고리즘으로<br/>최소 Hyperedge 검색"]
        OG1 --> OG2
    end

    subgraph KGRAG["KG-RAG"]
        direction TB
        KG1["DPR (비정형 텍스트)<br/>+ GNN (KG 경로)"]
        KG2["경로 어텐션으로<br/>이중 채널 결과 통합"]
        KG1 --> KG2
    end

    subgraph KG2["KG2RAG"]
        direction TB
        K21["KG 구조로<br/>검색 쿼리 재구성"]
        K22["관련 서브그래프 추출하여<br/>컨텍스트로 제공"]
        K21 --> K22
    end

    OGRAG --> |"공통: 도메인 온톨로지/KG를<br/>RAG에 구조적 통합"| RESULT["사실 정확도 향상<br/>환각 감소"]
    KGRAG --> RESULT
    KG2 --> RESULT

    style OGRAG fill:#16A085,color:#fff
    style KGRAG fill:#16A085,color:#fff
    style KG2 fill:#16A085,color:#fff
...
**A. 등장배경**
기존 RAG 시스템은 벡터 유사도 기반 검색에 의존하여, 도메인의 **구조적 지식(엔티티 간 계층, 관계, 제약조건)**을 활용하지 못했다. 이로 인해 전문 도메인(의료, 금융, 법률)에서 용어 체계가 반영되지 않고, 검색된 청크의 맥락 부족으로 **사실 환각(hallucination)**이 빈번했다. Graph RAG가 지식그래프를 도입했으나, 도메인 전문가가 정의한 **형식적 온톨로지(formal ontology)**를 체계적으로 활용하는 접근은 부재했다.

**B. 핵심 개념**
온톨로지(Ontology)는 도메인의 개념, 엔티티, 관계, 제약조건을 형식적으로 정의하는 지식 표현 체계이다. Ontology-Enhanced RAG는 이 구조화된 지식을 RAG 파이프라인의 인덱싱·검색·생성 전 단계에 통합한다:

| 접근법 | 온톨로지 활용 방식 | 핵심 메커니즘 |
|--------|------------------|-------------|
| **OG-RAG** | 도메인 온톨로지로 문서를 하이퍼그래프 구조화 | 각 하이퍼엣지가 온톨로지 기반 사실 클러스터를 캡슐화, 최적화 알고리즘으로 최소 집합 검색 |
| **KG-RAG** | 지식그래프를 이중 채널 검색에 통합 | DPR(비정형) + GNN(KG 경로) 이중 채널, 경로 어텐션으로 엔티티-관계 체인 필터링 |
| **KG2RAG** | KG 구조로 검색 쿼리 재구성 | 관련 서브그래프를 추출하여 생성 모델에 구조적 컨텍스트 제공 |
| **FAIR GraphRAG** | OWL 온톨로지로 FAIR 원칙 준수 | 상호운용성·재사용성 보장, 이기종 KG 통합 가능 |

**C. 기술적 기여**
OG-RAG는 4개 LLM 대상 실험에서 다음과 같은 성능 향상을 달성했다 (Sharma et al., EMNLP 2025):

| 지표 | 개선율 |
|------|--------|
| 정확한 사실 재현율 (Recall) | **+55%** |
| 응답 정확도 | **+40%** |
| 사실 기반 추론 정확도 | **+27%** |
| 응답 귀인(Attribution) 속도 | **+30%** |

또한 온톨로지 기반 KG 구축 비교 연구(arXiv:2511.05991)에서는:
- 관계형 DB에서 추출한 온톨로지 기반 KG가 텍스트 코퍼스 기반과 **동등한 성능**을 보이면서 **비용 효율적**
- 온톨로지 기반 KG + 청크 정보 결합 시, 순수 벡터 검색 베이스라인을 **상당히 초월**
- GraphRAG 등 최신 프레임워크와 경쟁적 성능 달성

**D. 산업별 적용 현황**

기업에서의 온톨로지 + RAG 적용은 규제 산업을 중심으로 빠르게 확산 중이다 (2025-2026 기준):

| 산업 | 활용 방식 | 활용 온톨로지 |
|------|-----------|-------------|
| **의료/제약** | 임상 의사결정 지원, 약물 상호작용 검증 | SNOMED CT, UMLS, MeSH |
| **금융** | 컴플라이언스 자동 검증, 규정 해석 | FIBO (Financial Industry Business Ontology) |
| **법률** | 판례 검색, 법률 QA 시스템 | 법률 용어 체계 + 판례 관계 그래프 |
| **제조** | 음성 제어 유지보수 어시스턴트 | 산업 설비 온톨로지 |
| **교육** | 지능형 교육 QA (KA-RAG) | 교육과정 온톨로지 |
| **재난 대응** | 비상 대응 의사결정 지원 | 지진 재해 도메인 온톨로지 |

기업 ROI: 지식그래프 + RAG 시스템 도입 기업들이 2024-2025년 기준 **300-320% ROI**를 달성한 것으로 보고됨. 규제 산업용 프리빌트 지식 런타임이 **50% 이상 시장 점유**를 기록 중이다.

**E. 한계점**

- **OWL 온톨로지 구축 복잡성**: 자동 구축 기술이 아직 미성숙하며, 도메인 전문가의 수작업이 필요한 경우가 많음
- **온톨로지 유지보수 비용**: 도메인 지식 변경 시 온톨로지 업데이트와 KG 재구축이 필요
- **기술 진입 장벽**: OWL-DL, SPARQL 등 시맨틱 웹 기술 스택의 학습 곡선이 높음
- **확장성 문제**: 대규모 온톨로지의 추론(reasoning) 성능이 실시간 서비스에 제약
- **표준화 부재**: 산업별 온톨로지 통합 표준이 아직 확립 중

**F. 향후 전망 (2026~)**

- 산업 컨소시엄이 공유 지식그래프와 온톨로지를 공동 유지관리하는 모델 확산
- LLM 기반 온톨로지 자동 학습(OntoRAG 등)으로 구축 비용 대폭 절감
- Agentic RAG + Ontology의 결합: 에이전트가 온톨로지를 도구(tool)로 활용하여 도메인 추론 수행
- FAIR 원칙 기반 GraphRAG로 이기종 지식그래프 간 상호운용성 확보
...
| **OG-RAG**        | 2024~2025 | 온톨로지 기반 하이퍼그래프 최적화 검색              | 사실 귀인 기반 생성         | 도메인 온톨로지로 사실 지식 클러스터 구조화             | 사실 재현율 +55%, 정확도 +40%         |
| **KG-RAG**        | 2025      | DPR + GNN 이중 채널 검색                   | KG 경로 주입 생성         | 경로 어텐션 기반 엔티티-관계 체인 필터링              | 의미·구조 검색 통합                    |
...
flowchart TB
    ...
    subgraph Gen3_5["3.5세대: Ontology-Enhanced RAG (2024~2025)"]
        direction LR
        G35A["도메인 온톨로지 기반 구조적 검색"]
        G35B["OG-RAG"]
        G35C["KG-RAG"]
        G35D["KG2RAG"]
        G35E["FAIR GraphRAG"]
...
### 3.5세대: Ontology-Enhanced RAG (2024~2025)

**특징**: 도메인 온톨로지를 RAG 파이프라인에 구조적으로 통합하여 사실 정확도를 극대화

Graph RAG가 KG 구축의 가능성을 열었다면, Ontology-Enhanced RAG는 도메인 전문가가 정의한 **형식적 온톨로지(formal ontology)**를 체계적으로 활용하여 검색과 생성의 사실적 기반을 강화한다:

- **구조적 인덱싱**: 문서를 온톨로지 기반 하이퍼그래프로 구조화하여 사실 지식 클러스터를 형성 (OG-RAG)
- **이중 채널 검색**: 벡터 검색(비정형)과 GNN 기반 그래프 검색(구조적)을 동시에 수행 (KG-RAG)
- **쿼리 재구성**: KG 구조를 활용하여 검색 쿼리를 의미적으로 확장 (KG2RAG)
- **상호운용성**: OWL 온톨로지와 FAIR 원칙으로 이기종 KG 통합 (FAIR GraphRAG)

이 세대는 특히 **의료(SNOMED CT), 금융(FIBO), 법률** 등 규제 산업에서 기존 산업 표준 온톨로지를 활용하여 높은 ROI를 달성하고 있다. 온톨로지 기반 접근은 사실 재현율 +55%, 응답 정확도 +40%의 개선을 보이며 (OG-RAG, EMNLP 2025), Agentic RAG의 도구(tool)로도 통합될 수 있는 확장성을 가진다.
...
> 모든 논문은 2026년 3월 기준 arXiv, ACM DL, ACL Anthology, OpenReview, Nature 등에서 실재를 개별 검증하였다.
...
| 16  | Singh, A., Ehtesham, A., Kumar, S., & Khoei, T. T. (2025). *Agentic Retrieval-Augmented Generation: A Survey on Agentic RAG*.                                                                                                                                                          | ✅     |
...
| 17  | Sharma, K., Kumar, P., & Li, Y. (2025). *OG-RAG: Ontology-Grounded Retrieval-Augmented Generation for Large Language Models*. **EMNLP 2025**.                                                                                                                                           | ✅     |
|     | [ACL Anthology](https://aclanthology.org/2025.emnlp-main.1674/) · [arXiv:2412.15235](https://arxiv.org/abs/2412.15235) · [GitHub](https://github.com/microsoft/ograg2)                                                                                                                 |       |
| 18  | Wang, S., Yang, H. & Liu, W. (2025). *Research on the construction and application of retrieval enhanced generation (RAG) model based on knowledge graph*. **Scientific Reports**, 15, 40425.                                                                                            | ✅     |
|     | [Nature](https://www.nature.com/articles/s41598-025-21222-z) · [PubMed](https://pubmed.ncbi.nlm.nih.gov/41253865/)                                                                                                                                                                      |       |
| 19  | Jiang et al. (2025). *Knowledge Graph-Guided Retrieval Augmented Generation*. **NAACL 2025**.                                                                                                                                                                                             | ✅     |
|     | [arXiv:2502.06864](https://arxiv.org/abs/2502.06864) · [GitHub](https://github.com/nju-websoft/KG2RAG)                                                                                                                                                                                   |       |
| 20  | Peng et al. (2025). *Graph Retrieval-Augmented Generation: A Survey*. **ACM Transactions on Information Systems**.                                                                                                                                                                        | ✅     |
|     | [ACM DL](https://dl.acm.org/doi/10.1145/3777378) · [arXiv:2408.08921](https://arxiv.org/abs/2408.08921)                                                                                                                                                                                 |       |
| 21  | (2025). *Ontology Learning and Knowledge Graph Construction: A Comparison of Approaches and Their Impact on RAG Performance*.                                                                                                                                                             | ✅     |
|     | [arXiv:2511.05991](https://arxiv.org/abs/2511.05991)                                                                                                                                                                                                                                     |       |
| 22  | Zhang et al. (2025). *A Survey of Graph Retrieval-Augmented Generation for Customized Large Language Models*.                                                                                                                                                                             | ✅     |
|     | [arXiv:2501.13958](https://arxiv.org/abs/2501.13958) · [GitHub](https://github.com/DEEP-PolyU/Awesome-GraphRAG)                                                                                                                                                                         |       |
| 23  | Flüh et al. (2025). *FAIR GraphRAG: A Retrieval-Augmented Generation Approach*. **RWTH Aachen**.                                                                                                                                                                                          | ✅     |
|     | [PDF](https://www.comsys.rwth-aachen.de/publication/2025/2025_flueh_fair_graph-retrieval-augmented-generation/2025_flueh_FAIR_graph-retrieval-augmented-generation.pdf)                                                                                                                   |       |
| 24  | *RAG and Ontologies for Information Retrieval: A Literature Review*. University of Toronto, 2025.                                                                                                                                                                                         | ✅     |
|     | [PDF](https://eil.mie.utoronto.ca/wp-content/uploads/2025/09/ONTOLLM_2025_Aug20v11.pdf)                                                                                                                                                                                                 |       |
