...
- 예: `api/auth` → `api-auth`
- 예: `datasource/aurora` → `datasource-aurora`
...
Flyway는 의존성과 `src/main/resources/db/migration/` 디렉토리만 준비돼 있고 마이그레이션 파일은 아직 없습니다. 스키마는 외부에서 관리하며 `ddl-auto: none`으로 동작합니다.
...
| `mysql-chatbot` | 3310 | chatbot | api/chatbot, api/agent |

```bash
# Docker Compose 실행 (MySQL 4개 + Kafka + 모니터링 스택. Redis·MongoDB 컨테이너는 없음)
docker compose up -d
...
export MONGODB_ATLAS_DATABASE=tech_n_ai
...
# Cohere Re-Ranking·Google 웹 검색 (선택)
...
export SLACK_WEBHOOK_ENABLED=false
export SLACK_BOT_TOKEN=your-slack-bot-token
export SLACK_BOT_ENABLED=false
export SLACK_DEFAULT_CHANNEL=#emerging-tech

# 내부 API·에이전트 스케줄러
export EMERGING_TECH_INTERNAL_API_KEY=your-internal-api-key
export AGENT_SCHEDULER_ENABLED=false

# 로컬 MySQL (Docker Compose)
export MYSQL_ROOT_PASSWORD=root1234
export MYSQL_USER=admin
export MYSQL_PASSWORD=admin1234
```
...
- Dev/Beta/Prod 환경: `http://api-{service}-service:8080` (환경별 설정 파일에 정의된 내부 서비스 주소)
