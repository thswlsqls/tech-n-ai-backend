...
| `external_id` | String | 외부 식별자, unique 인덱스 (중복 검사에 사용) |
...
`provider`, `url`, `published_at`, `status`에 인덱스가 있고, `external_id`는 unique 인덱스입니다.
...
| 변수명 | 설명 | 필수 |
|--------|------|------|
| `EMERGING_TECH_INTERNAL_API_KEY` | 내부 API 인증 키 | Yes |
